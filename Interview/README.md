# YC Interview Prep

YC interviews are ~10 minutes, rapid-fire. Partners interrupt constantly. Every answer must be ≤ 30 seconds and land a clear point.

## Folder Structure

| Folder | What goes there |
|--------|----------------|
| [company/](company/) | One-liner, mission, elevator pitch |
| [product/](product/) | How it works, technical approach, demo notes, tech stack rationale |
| [market/](market/) | TAM/SAM/SOM, market dynamics, why now |
| [team/](team/) | Founder bios, why us, commitment |
| [traction/](traction/) | Metrics, customer conversations, lab results |
| [competition/](competition/) | Competitive landscape, moat, differentiation |
| [business-model/](business-model/) | Revenue model, pricing, go-to-market |
| [qa-prep/](qa-prep/) | Scripted answers to the hardest YC questions |
| [policy-and-funding/](policy-and-funding/) | Regulatory pathways, international scaling, political risk, federal grants |
| [vision/](vision/) | 10-year story, endgame options, why we win |
| [pivot-playbook.md](pivot-playbook.md) | What we do if each failure mode hits |
| [series-a.md](series-a.md) | Series A milestone, target investors, raise narrative |
| [numbers-cheatsheet.md](numbers-cheatsheet.md) | Every number to memorize cold — one page |

## Interview Format

- 10 minutes total, ~15–20 questions
- 2–4 YC partners in the room
- They will cut you off — brevity is survival
- They will probe inconsistencies — know your numbers cold
- End with a clear ask: what you need, what you'll do with it

## Key Documents

- [Numbers Cheat Sheet](numbers-cheatsheet.md) — start here, memorize everything on this page
- [Tech Stack Rationale](product/tech-stack.md) — every tool, why we chose it, what we ruled out
- [Protecting Peptides Against the Elements](product/protecting-peptides-against-elements.md) — field stability, formulation, and residue-engineering answer
- [Peptide Delivery Through Barriers](product/peptide-delivery-through-barriers.md) — cell wall, cuticle, and gut-delivery strategy
- [One-Liner & Pitch](company/one-liner.md) — memorize this
- [Q&A Master Sheet](qa-prep/questions.md) — drill daily out loud
- [Market Sizing](market/tam-sam-som.md) — know the math
- [Competitive Landscape](competition/landscape.md) — know every player
- [Co-Founder Dynamics](team/founders.md) — fill in the personal answers before the interview
- [10-Year Vision](vision/ten-year-story.md) — commit to an endgame
- [Policy & Funding](policy-and-funding/overview.md) — regulatory strategy, political risks, SBIR grants
- [Pivot Playbook](pivot-playbook.md) — have a specific answer for every failure mode
- [Series A](series-a.md) — know exactly what unlocks the next round
